...
//        images.add(inputFrame);
    ...
    private static Mat getCircleFromImage(Mat inputFrame, Point center, int radius) {
        Point left = new Point(center.x - radius, center.y + radius);
        Point right = new Point(center.x + radius, center.y - radius);
        Rect rect = new Rect((int) left.x, (int) left.y, (int)(right.x - left.x + 1), (int)(right.y - left.y + 1));
        return inputFrame.submat(rect);
    }
